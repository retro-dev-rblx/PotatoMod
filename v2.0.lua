...
credits to NicePotato (me), Ayray, and Cristiano]]
	...
	if Explorer then
		...
		local ExplorerListOutline = newdefault(Explorer,"ExplorerListOutline")
		if ExplorerListOutline then
			local ScrollingFrame = newreg(ExplorerListOutline,"ScrollingFrame")
			if ScrollingFrame then
				regdynamic(dynamicExplorerHandlerConnect,
					dynamicExplorerHandlerDisconnect)
			end
			newscrollbar(ExplorerListOutline,"ScrollbarBackground")
		end
		local SearchListOutline = newdefault(Explorer,"SearchListOutline")
		if SearchListOutline then
			local ScrollingFrame = newdefault(SearchListOutline,"ScrollingFrame")
			if ScrollingFrame then
				regdynamic(dynamicExplorerHandlerConnect,
					dynamicExplorerHandlerDisconnect)
			end
			newscrollbar(SearchListOutline,"ScrollbarBackground")
		end
		newdefault(Explorer, "SearchBar")
	end
	...
	local function dynamicPropertiesHandlerConnect(instance, entry)
		...
		entry[8] = {} -- Category Template Child Event
		...
		local function recolorTextLabel(TextLabel)
			local function recolorValueHalf(color)
				...
				for _,child in pairs(ValueHalf:GetChildren()) do
					if child:IsA("TextBox") or child:IsA("TextButton") or child.Name == "EnumText" or child.name == "BrickColorText" then
		...
		local function hookPropertyName(TextLabel)
			...
			if TextLabel:IsA("TextLabel") then
				...
				entry[5][self_entry] = {TextLabel,renderstepped}
		...
		local function hookProperty(instance)
			...
			for _,child in pairs(instance.Background:GetChildren()) do
			...
			recolorPropertyBG(instance.Background)
			...
			local bgConnection = instance.Background:GetPropertyChangedSignal("BackgroundColor3"):Connect(function()
				recolorPropertyBG(instance.Background)
			end)
		...
		local function hookCategoryTemplate(child)
			...
			local self_entry = #entry[8]+1

			local connection = child.ChildAdded:Connect(function(child)
				hookChild(child)
			end)

			entry[self_entry] = connection

			child.Destroying:Connect(function()
				entry[self_entry] = nil
			end)
		end
	...
	local function dynamicPropertiesHandlerDisconnect(instance, entry)
		...
		for _,CategoryTemplateConnection in pairs(entry[8]) do
			CategoryTemplateConnection:Disconnect()
		end
	end
	...
	if Properties then
		...
		if PropertiesScript then
			local Templates = newreg(PropertiesScript,"Templates")
			if Templates then
				local EnumList = newreg(Templates,"EnumList")
				if EnumList then
					regprop("ImageTransparency",1)
					regprop("BackgroundTransparency",0)
					regtheme("BackgroundColor3",theme.ol)
				...
				local PropertyBrickColorPalette = newreg(Templates,"PropertyBrickColorPalette")
	...
	if Topbar then
		...
		if TitleBar then
			...
			local Border8 = newreg(TitleBar,"Border8")
			if Border8 then
...
if script and not _is_exploit_env then -- Real PotatoMod, do startup stuffs
